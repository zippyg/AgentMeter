...
struct StatusMenuTests {
    ...
    func `overview tab omits contextual provider actions`() {
        ...
        #expect(titles.contains("About AgentMeter"))
...
extension StatusMenuTests {
    ...
    func `provider config changes preserve status item instances`() throws {
        ...
        #expect(codexItem.autosaveName == "com.zain.agentmeter.menu.codex")
        ...
        #expect(controller.statusItems[.gemini]?.autosaveName == "com.zain.agentmeter.menu.gemini")
    }
...
extension StatusMenuTests {
    @Test
    func `overview tab renders AgentMeter summary without provider detail rows`() {
        self.disableMenuCardsForTesting()
        let settings = self.makeSettings()
        settings.statusChecksEnabled = false
        settings.refreshFrequency = .manual
        ...
        #expect(ids.contains("agentMeterSummary"))
        #expect(ids.contains(where: { $0.hasPrefix("overviewRow-") }) == false)
        #expect(ids.contains("menuCard") == false)
    }

    @Test
    func `overview tab ignores stored subset for summary-only layout`() {
        self.disableMenuCardsForTesting()
        let settings = self.makeSettings()
        settings.statusChecksEnabled = false
        settings.refreshFrequency = .manual
    ...
    @Test
    func `overview summary has no provider-row action in rendered mode`() throws {
        StatusItemController.menuCardRenderingEnabled = true
        StatusItemController.setMenuRefreshEnabledForTesting(false)
        ...
        let summary = try #require(menu.items.first {
            ($0.representedObject as? String) == "agentMeterSummary"
        })
        #expect(summary.action == nil)
        #expect(summary.submenu == nil)
