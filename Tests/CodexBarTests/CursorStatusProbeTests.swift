...
extension CursorStatusProbeTests {
    ...
    func `fetch uses Cursor app local auth when browser cookies are unavailable`() async throws {
        ...
        CursorStatusProbeStubURLProtocol.setHandler { request in
            let requestURL = try #require(request.url)
            guard requestURL.host == "cursor-web.test" else {
        ...
        #expect(CursorStatusProbeStubURLProtocol.requestPaths(host: "cursor-web.test").sorted() == [
            "/api/auth/me",
            "/api/usage",
            "/api/usage-summary",
        ])
...
final class CursorStatusProbeStubURLProtocol: URLProtocol {
    ...
    static func requestPaths(host: String) -> [String] {
        self.lock.lock()
        defer { Self.lock.unlock() }
        return self.state.requests.compactMap { request in
