...
struct ClaudeBaselineCharacterizationTests {
    ...
    private func missingStatusLineCacheEnv(_ env: [String: String] = [:]) -> [String: String] {
        var merged = env
        merged[ClaudeStatusLineUsageCache.overridePathEnvironmentKey] = FileManager.default.temporaryDirectory
            .appendingPathComponent("missing-claude-statusline-\(UUID().uuidString).json")
            .path
        return merged
    }
    ...
    @Test
    func `app auto pipeline order is OAuth then status-line cache then CLI then web`() async {
        ...
        let env = self.missingStatusLineCacheEnv([
        ...
        #expect(strategyIDs == ["claude.oauth", "claude.statusline-cache", "claude.cli", "claude.web"])
    }

    @Test
    func `CLI auto pipeline order is web then status-line cache then CLI`() async {
        let settings = ProviderSettingsSnapshot.make(claude: .init(
            usageDataSource: .auto,
        ...
        #expect(strategyIDs == ["claude.web", "claude.statusline-cache", "claude.cli"])
    }

    @Test
    func `explicit CLI pipeline attempts CLI after status-line cache when planner marks CLI unavailable`() async {
        let settings = ProviderSettingsSnapshot.make(claude: .init(
            usageDataSource: .cli,
            webExtrasEnabled: false,
            cookieSource: .off,
        ...
        #expect(strategies.map(\.id) == ["claude.statusline-cache", "claude.cli"])
        #expect(await strategies[0].isAvailable(context) == false)
        #expect(await strategies[1].isAvailable(context))
    }

    @Test
    func `auto pipeline records unavailable planned steps including status-line cache`() async {
        let settings = ProviderSettingsSnapshot.make(claude: .init(
            usageDataSource: .auto,
            webExtrasEnabled: true,
            cookieSource: .off,
            manualCookieHeader: nil))
        let env = self.missingStatusLineCacheEnv(["CLAUDE_CLI_PATH": "/definitely/missing/claude"])
        ...
        await self.withNoOAuthCredentials {
            await ClaudeCLIResolver.withResolvedBinaryPathOverrideForTesting("/definitely/missing/claude") {
                ...
                #expect(outcome.attempts.map(\.wasAvailable) == [false, false, false, false])
    ...
    func `auto pipeline CLI uses planned environment for execution`() async throws {
        ...
        let env = self.missingStatusLineCacheEnv(["CLAUDE_CLI_PATH": stubCLIPath])
        ...
        expectedStrategyID: String) async
    {
        let strategyIDs = await self.strategyIDs(
            runtime: .app,
            sourceMode: sourceMode,
            env: self.missingStatusLineCacheEnv())
        let expected = sourceMode == .cli ? ["claude.statusline-cache", expectedStrategyID] : [expectedStrategyID]
