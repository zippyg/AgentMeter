...
struct OpenAIDashboardParserTests {
    ...
    func `parses signed in email from client bootstrap HTML`() {
        ...
        {"authStatus":"logged_in","session":{"user":{"email":"studpete@example.com"}}}
        </script>
        </body>
        </html>
        """
        #expect(OpenAIDashboardParser.parseSignedInEmailFromClientBootstrap(html: html) == "studpete@example.com")
