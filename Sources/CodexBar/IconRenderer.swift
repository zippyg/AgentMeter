...
enum IconRenderer {
    ...
    static func makeAgentMeterIdentityIcon(
        stale: Bool,
        blink: CGFloat = 0,
        wiggle: CGFloat = 0,
    ...
    private static func drawAgentMeterRobot(fill: NSColor, blink: CGFloat, wiggle: CGFloat) {
        ...
        func fillRect(x: Int, y: Int, w: Int, h: Int, radius: Int = 0) {
            let rect = Self.grid.rect(x: x, y: y, w: w, h: h)
            let path = radius > 0
                ? NSBezierPath(
                    roundedRect: rect,
                    xRadius: Self.grid.pt(radius),
                    yRadius: Self.grid.pt(radius))
