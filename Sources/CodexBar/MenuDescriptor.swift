...
struct MenuDescriptor {
        ...
        includeContextualActions: Bool = true) -> MenuDescriptor
    {
        ...
        sections.append(Self.phoneSnapshotSection())
        sections.append(Self.metaSection(updateReady: updateReady))

        return MenuDescriptor(sections: sections)
    }

    static func phoneSnapshotSection(
        fileURL: URL = AgentMeterPhoneSnapshotStore.defaultURL(),
        fileExists: Bool? = nil) -> Section
    {
        let exists = fileExists ?? FileManager.default.fileExists(atPath: fileURL.path)
        return Section(entries: [
            .submenu(L("iPhone Snapshot"), MenuActionSystemImage.phone.rawValue, [
                ...
                SubmenuItem(
                    ...
                    isEnabled: exists),
                SubmenuItem(
                    title: L("Reveal Snapshot in Finder"),
                    action: .revealPhoneSnapshot,
                    isEnabled: exists),
            ]),
        ])
    }
    ...
    private static func metaSection(updateReady: Bool) -> Section {
        ...
        entries.append(contentsOf: [
            ...
            .action("About \(AgentMeterProductIdentity.displayName)", .about),
            .action(L("Quit"), .quit),
        ])
        return Section(entries: entries)
    }
