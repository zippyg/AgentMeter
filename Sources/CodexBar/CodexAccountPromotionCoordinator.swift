    ...
    static func mapUserFacingError(_ error: Error) -> CodexSystemAccountPromotionUserFacingError {
        ...
        if let error = error as? CodexAccountPromotionError {
            ...
            case .targetManagedAccountNotFound:
                L("That account is no longer available in AgentMeter. Refresh the account list and try again.")
            case .targetManagedAccountAuthMissing:
                L("AgentMeter could not find saved auth for that account. Re-authenticate it and try again.")
            case .targetManagedAccountAuthUnreadable:
                L("AgentMeter could not read saved auth for that account. Re-authenticate it and try again.")
            case .liveAccountUnreadable:
                L("AgentMeter could not read the current system account on this Mac.")
            case .liveAccountMissingIdentityForPreservation:
                L("AgentMeter could not safely preserve the current system account before switching.")
            case .liveAccountAPIKeyOnlyUnsupported:
                L("AgentMeter can't replace a system account that is signed in with an API key only setup.")
            case .displacedLiveManagedAccountConflict:
                L(
                    "AgentMeter found another managed account that already uses the current system account. " +
                        "Resolve the duplicate account before switching.")
            case .displacedLiveImportFailed:
                L("AgentMeter could not save the current system account before switching.")
            case .managedStoreCommitFailed:
                L("AgentMeter could not update managed account storage.")
            case .liveAuthSwapFailed:
                L("AgentMeter could not replace the live Codex auth on this Mac.")
            }
