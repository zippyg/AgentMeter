...
final class UsageStore {
    ...
    @ObservationIgnored var _test_agentMeterSnapshotLoadOverride: (@MainActor () -> AgentMeterSnapshot?)?
    @ObservationIgnored var _test_agentMeterSnapshotSaveOverride: (@MainActor (AgentMeterSnapshot) async -> Void)?
    @ObservationIgnored var _test_agentMeterPhoneSnapshotSaveOverride: (@MainActor (
        AgentMeterPhoneSnapshot) async -> Void)?
    ...
    @ObservationIgnored var rateLimitLastUsedPercent: [UsageProvider: Double] = [:]
...
extension UsageStore {
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            let claudeSettings = self.settings.claudeSettingsSnapshot(tokenOverride: nil)
            let environment = ProviderRegistry.makeEnvironment(
                base: self.environmentBase,
                provider: .claude,
                settings: self.settings,
            ...
            return ClaudeDebugLogConfiguration(
                ...
                usageDataSource: claudeSettings.usageDataSource == .auto
                    ? fallbackUsageDataSource
                    : claudeSettings.usageDataSource,
                cookieSource: claudeSettings.cookieSource == .auto
                    ? fallbackCookieSource
                    : claudeSettings.cookieSource,
                cookieHeader: claudeSettings.manualCookieHeader ?? fallbackCookieHeader,
                keepCLISessionsAlive: self.settings.debugKeepCLISessionsAlive)
        }
    }
