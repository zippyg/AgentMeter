...
final class StatusItemController: NSObject, NSMenuDelegate, StatusItemControlling {
    ...
    private nonisolated static let statusItemAccessibilityTitle = AgentMeterProductIdentity.displayName
    private nonisolated static let statusItemAccessibilityIdentifierPrefix =
        AgentMeterProductIdentity.statusItemAccessibilityIdentifierPrefix
    ...
    enum StatusItemIdentity {
        ...
        var autosaveName: String {
            switch self {
            case .merged:
                AgentMeterProductIdentity.statusItemAutosaveRoot
            case let .provider(provider):
                "\(AgentMeterProductIdentity.statusItemAutosaveRoot).\(provider.rawValue)"
            }
        }

        var legacyAutosaveNames: [String] {
            switch self {
            case .merged:
                var names = [
                    "\(AgentMeterProductIdentity.legacyStatusItemAutosavePrefix)-merged",
                    AgentMeterProductIdentity.previousLocalStatusItemAutosaveRoot,
                    AgentMeterProductIdentity.previousMacBundleIdentifier,
                ]
                names.append(contentsOf: AgentMeterProductIdentity.previousVersionedStatusItemAutosaveRoots.map {
                    "\($0)-merged"
                })
                return names
            case let .provider(provider):
                var names = [
                    "\(AgentMeterProductIdentity.legacyStatusItemAutosavePrefix)-\(provider.rawValue)",
                    "\(AgentMeterProductIdentity.previousLocalStatusItemAutosaveRoot).\(provider.rawValue)",
                    "\(AgentMeterProductIdentity.previousMacBundleIdentifier).\(provider.rawValue)",
                ]
                names.append(contentsOf: AgentMeterProductIdentity.previousVersionedStatusItemAutosaveRoots.map {
                    "\($0)-\(provider.rawValue)"
                })
    ...
    var menuBarVisibilityWatchdogTask: Task<Void, Never>?
    var lastMenuBarVisibilityWatchdogSignature: String?
    var lastMenuBarStatusItemRecoveryAt: Date?
    ...
    private static func makeStatusItem(
        ...
        -> NSStatusItem
    {
        let usesAutosaveName = AgentMeterLaunchMode(
            environment: ProcessInfo.processInfo.environment)
            .usesStatusItemAutosave
        if usesAutosaveName {
            let persistenceDefaults = self.statusItemPersistenceDefaults(fallback: defaults)
            MenuBarStatusItemDefaultsRepair.migrateLegacyStatusItemDefaults(
                defaults: persistenceDefaults,
                autosaveName: identity.autosaveName,
                legacyAutosaveNames: identity.legacyAutosaveNames)
            MenuBarStatusItemPlacementPreflight.prepare(
                defaults: persistenceDefaults,
                autosaveName: identity.autosaveName,
                legacyDefaultItemIndex: legacyDefaultItemIndex)
        }
        let item = statusBar.statusItem(withLength: self.initialStatusItemLength(identity: identity))
        if usesAutosaveName {
    ...
    static func statusItemPersistenceDefaults(fallback: UserDefaults) -> UserDefaults {
        fallback
    }

    private static func initialStatusItemLength(identity: StatusItemIdentity) -> CGFloat {
        switch identity {
        case .merged:
        ...
        observeProviderConfigNotifications: Bool = !SettingsStore.isRunningTests)
    {
        ...
        let repairedStatusItemVisibilityKeys = MenuBarStatusItemDefaultsRepair
            .repairHiddenVisibilityDefaultsIfNeeded(
                defaults: Self.statusItemPersistenceDefaults(fallback: settings.userDefaults))
        ...
        self.updateIcons()
        self.writeStatusItemDiagnostics(reason: "startup-initial")
        ...
        self.startMenuBarVisibilityWatchdog()
...
extension StatusItemController {
    ...
    var visibilityRecoveryStatusItems: [NSStatusItem] {
        [self.statusItem] + Array(self.statusItems.values)
    }

    @discardableResult
    func recoverMissingMergedStatusItemWindow(
        reason: String,
        missingAutosaveNames: [String],
        windowDiagnostics: String,
        now: Date = Date())
        -> Bool
    {
        guard self.shouldMergeIcons,
              let autosaveName = self.statusItem.autosaveName,
              !autosaveName.isEmpty,
              missingAutosaveNames.contains(autosaveName)
        else {
            return false
        }
        // The "missing" verdict comes from MenuBarStatusItemWindowProbe, which matches windows by
        // kCGWindowName. Recent macOS (Tahoe and later) redacts CoreGraphics window names, so the
        // probe returns no match for a perfectly materialized item and reports it as missing every
        // watchdog tick. Recreating a healthy item makes the menu-bar icon visibly flash. Trust
        // AppKit's authoritative view of OUR item instead: if it has a window in the menu-extra
        // region on a screen, it is present, so do not recreate it. Genuine loss (the original
        // materialization bug) still leaves hasWindow false, so real recovery is preserved.
        let appKitSnapshot = MenuBarVisibilityWatcher.visibilitySnapshot(self.statusItem)
        if MenuBarVisibilityWatcher.statusItemAppearsMaterialized(appKitSnapshot) {
            return false
        }
        guard !self.isMergedMenuOpen else { return false }
        if let last = self.lastMenuBarStatusItemRecoveryAt,
           now.timeIntervalSince(last) < 8
        {
            return false
        }

        self.lastMenuBarStatusItemRecoveryAt = now
        self.menuLogger.error(
            "Recovering missing AgentMeter menu-bar status item window",
            metadata: [
                "reason": "\(reason)",
                "autosaveName": autosaveName,
                "missingAutosaveNames": missingAutosaveNames.joined(separator: ","),
        ...
        Task { @MainActor [weak self] in
            ...
            self?.writeStatusItemDiagnostics(reason: "\(reason)-status-item-recovery-settled")
        }
        return true
    }
}
