...
extension StatusItemController {
    private struct BrandPercentIconRequest {
    ...
    private struct ProviderBrandPercentIconRequest {
        let item: NSStatusItem
        let button: NSStatusBarButton
        let provider: UsageProvider
        let style: IconStyle
        ...
        bypassMergedMenuTrackingDeferral: Bool = false) -> Bool
    {
        ...
        if let morphProgress {
            ...
            let image = IconRenderer.makeAgentMeterIdentityIcon(
                stale: stale,
                blink: blink,
                wiggle: wiggle,
                statusIndicator: statusIndicator)
    ...
    private func currentStatusIndicator() -> ProviderStatusIndicator {
    ...
    private func applyBrandPercentIconIfNeeded(_ request: BrandPercentIconRequest) -> Bool? {
        guard self.settings.menuBarShowsBrandIconWithPercent else { return nil }
        ...
        if self.shouldSkipMergedIconRender(signature) {
            self.applyAgentMeterMenuBarBadge(
                item: request.item,
                button: request.button,
                baseImage: baseImage,
                displayText: displayText,
                warningFlash: request.warningFlash)
            self.noteIconPerfRender(skipped: true)
            return true
        }
        self.applyAgentMeterMenuBarBadge(
    ...
    func agentMeterMenuBarDisplayText(for _: UsageProvider, snapshot _: UsageSnapshot?) -> String {
        "AM"
    }
    ...
    func applyIcon(for provider: UsageProvider, phase: Double?) -> Bool {
        ...
        {
            return self.applyProviderBrandPercentIcon(.init(
                item: item,
                button: button,
                provider: provider,
                style: style,
    ...
    private func applyAgentMeterMenuBarBadge(
        item: NSStatusItem,
        ...
        warningFlash: Bool)
    {
        let isMergedItem = item === self.statusItem
        let image: NSImage? = if isMergedItem {
            baseImage.map { baseImage in
                let copy = (baseImage.copy() as? NSImage) ?? baseImage
                copy.isTemplate = true
                return copy
            }
        } else {
            baseImage.flatMap { baseImage in
                warningFlash ? Self.quotaWarningFlashImage(base: baseImage) : baseImage
            }
        }
        self.setButtonImage(image, for: button)
        self.setButtonTitle(isMergedItem ? nil : displayText, for: button)
        if isMergedItem, button.contentTintColor != nil {
            button.contentTintColor = nil
        }
        let length = isMergedItem
            ? NSStatusItem.squareLength
            : Self.agentMeterNativeStatusItemLength(
                displayText: displayText,
                hasImage: image != nil)
        if abs(item.length - length) > 0.5 {
            item.length = length
        }
        let tooltip = isMergedItem || displayText == "AM"
            ? AgentMeterProductIdentity.displayName
            : "\(AgentMeterProductIdentity.displayName) \(displayText)"
        if button.toolTip != tooltip {
            button.toolTip = tooltip
        }
    }

    private func applyProviderBrandPercentIcon(_ request: ProviderBrandPercentIconRequest) -> Bool {
        let displayText = self.menuBarDisplayText(for: request.provider, snapshot: request.snapshot)
        let fallbackDisplayText = ProviderDescriptorRegistry.descriptor(for: request.provider).metadata.displayName
        let signature = [
            "mode=brandPercent",
            "provider=\(request.provider.rawValue)",
            "style=\(String(describing: request.style))",
        ...
        if self.shouldSkipProviderIconRender(provider: request.provider, signature: signature) {
            self.applyAgentMeterMenuBarBadge(
                item: request.item,
                button: request.button,
                baseImage: request.baseImage,
                displayText: displayText ?? fallbackDisplayText,
                warningFlash: request.warningFlash)
            self.noteIconPerfRender(skipped: true)
            return true
        }
    ...
    static func agentMeterNativeStatusItemLength(displayText: String, hasImage: Bool) -> CGFloat {
        let title = self.buttonTitle(displayText, hasImage: hasImage)
        let font = NSFont.systemFont(ofSize: NSFont.systemFontSize(for: .small), weight: .semibold)
        let textWidth = ceil((title as NSString).size(withAttributes: [.font: font]).width)
        let imageWidth: CGFloat = hasImage ? 18 : 0
        return min(112, max(42, textWidth + imageWidth + 18))
    }
    ...
    private func setButtonTitle(_ title: String?, for button: NSStatusBarButton) {
        ...
        let textColor = Self.agentMeterMenuBarTextColor(for: button)
        if button.contentTintColor != textColor {
            button.contentTintColor = textColor
        }
        let attributedTitle = NSAttributedString(
            string: value,
            attributes: value.isEmpty ? [:] : [
                .font: NSFont.systemFont(ofSize: NSFont.systemFontSize(for: .small), weight: .semibold),
                .foregroundColor: textColor,
            ])
        if !value.isEmpty || button.attributedTitle.string != attributedTitle.string {
    ...
    private static func agentMeterMenuBarTextColor(for _: NSStatusBarButton) -> NSColor {
