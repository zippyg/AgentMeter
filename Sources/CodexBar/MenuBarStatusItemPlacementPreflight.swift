...
enum MenuBarStatusItemPlacementPreflight {
    ...
    static let minimumPreferredPosition: Double = 24
    ...
    static func shouldClearPreferredPosition(_ value: Any, maximumPreferredPosition: Double?) -> Bool {
        ...
        if position < self.minimumPreferredPosition {
            return true
        }
    ...
    @discardableResult
    static func clearPreferredPositions(defaults: UserDefaults, autosaveNames: [String]) -> [String] {
        let keys = autosaveNames
            .map { self.preferredPositionKey(autosaveName: $0) }
